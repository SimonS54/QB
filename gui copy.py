...
class OCRWorker(QtCore.QThread):
    ...
    def process_text(self, text):
        ...
        logging.info(f"Extracted issue: {self.error}")
    ...
    def extract_issue(self, text):
        ...
        for sent in doc.sents:
            if "issue" in sent.text.lower():
                issue_sentences.append(sent.text.strip())

        if issue_sentences:
            for sentence in issue_sentences:
                if "issue" in sentence.lower():
                    return self.extract_specific_issue(sentence)
        return None

    def extract_specific_issue(self, sentence):
        doc = nlp(sentence)
        for token in doc:
            if "issue" in token.text.lower():
